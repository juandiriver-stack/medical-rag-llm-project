...
Responsabilidad única: encontrar y retornar información de pacientes
específicos. Detecta ID, cédula o nombre y ejecuta el SQL correcto.
No calcula estadísticas. No hace RAG.
"""
...
_PROMPT_PATH = Path(__file__).parent.parent / "prompts" / "agent_patient.md"
PATIENT_AGENT_PROMPT = _PROMPT_PATH.read_text(encoding="utf-8") if _PROMPT_PATH.exists() else ""
...
def _extraer_nombre(text: str) -> str | None:
    ...
    for _ in range(3):
        ...
        if nombre == antes:
            break
    nombre = re.sub(
        r"\s+(por favor|gracias|porfavor|ok).*$", "", nombre, flags=re.IGNORECASE
    )
    nombre = nombre.strip(".,?¿!¡ \t")
...
def _es_nombre_mayusculas(text: str) -> bool:
    ...
    if len(palabras) >= 2 and sum(
        1 for w in palabras if w.isupper() and len(w) > 1
    ) >= 2:
        return True
    return False
...
class PatientAgent:
    """
    Agente especializado en búsqueda de pacientes específicos.
    Detecta la entidad (ID / cédula / nombre) y ejecuta el SQL directo.
    """
    ...
    def execute(self, text: str, keywords: list) -> dict:
        """
        Detecta el tipo de identificador en el texto y busca al paciente.
        Prioridad: ID numérico > cédula > nombre > lista general.
        """
        ...
        # 3. Nombre en MAYÚSCULAS o con prefijo
        if _es_nombre_mayusculas(text):
            nombre = _extraer_nombre(text)
            if nombre:
                return self._por_nombre(nombre, text)

        # 4. Sin entidad específica → listar pacientes
        return self._listar(text)
    ...
    def _por_id(self, pid: int, text: str) -> dict:
        ...
        return {
            ...
            "context":    self._fmt(paciente, historial),
    ...
    def _por_cedula(self, cedula: str, text: str) -> dict:
        ...
        try:
            row = self.db.execute(
                sqlt("SELECT ID_PACIENTE FROM paciente "
                     "WHERE IDENTIFICACION = :c LIMIT 1"),
                {"c": cedula}
            ).fetchone()
            if row:
    ...
    def _por_nombre(self, nombre: str, text: str) -> dict:
        ...
        try:
            ...
            # Varios resultados → lista para que el usuario elija
            ...
            return {
                ...
                "context": (
                    ...
                    f"{lista}\n\n"
                    f"Escribe el ID del paciente para ver su historial completo."
                ),
                "intent": "buscar_paciente",
            }
    ...
    # ── Listar pacientes ──────────────────────────────────────────────
    def _listar(self, text: str) -> dict:
        data  = self.tool.list_pacientes(limit=15)
        items = data.get("items", [])
        filas = "\n".join(
            f"  • ID {p.get('id_paciente')} — "
            f"{p.get('nombre_completo','-')} | "
    ...
    def _fmt(paciente: dict, historial: dict) -> str:
        ...
        if items:
            ...
            for c in items[:10]:
                ...
                lines.append(
                    f"  [{fecha}] Motivo: {motivo} | Enfermedad: {enf}"
                )
        else:
